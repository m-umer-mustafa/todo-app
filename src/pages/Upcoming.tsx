...
export default function Upcoming() {
  ...
  const { getUpcomingTasks, toggleTask, deleteTask, tasks } = useTaskStore();
  const [displayedTasks, setDisplayedTasks] = useState(getUpcomingTasks());

  useEffect(() => {
    setDisplayedTasks(getUpcomingTasks());
  }, [tasks]);

  useEffect(() => {
    const onSearch = (e: Event) => {
      const { results } = (e as CustomEvent<{ results: Task[] | null }>).detail;
      setDisplayedTasks(results ?? getUpcomingTasks());
    };
